...
// Allowed MIME types and extensions for testimonial images
...
export async function POST(request) {
  ...
  try {
    ...
    const relativeUrl = `/uploads/testimonials/${uniqueFilename}`;

    return NextResponse.json(
      { message: "Image uploaded successfully", url: relativeUrl },
      { status: 200 }
    );
  } catch (error) {
    console.error("POST Admin Testimonial Image Upload API Error:", error);
    return NextResponse.json(
